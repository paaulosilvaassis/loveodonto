/**
 * Phase 10.9 — apply local 028–031 + validação SQL Contracts V2.
 *
 * Requer:
 ...
 *
 * Nunca: npx, link, db push, workdir supabase/ remoto.
 *
 *   node scripts/supabase/runLocalContractsV2Validation.mjs
 *   node scripts/supabase/runLocalContractsV2Validation.mjs --json
 */
...
const FIXTURE_REL = path.join('fixtures', 'contracts_v2_phase109_validation.sql');
...
export async function runLocalContractsV2Validation(options = {}) {
  ...
  if (!isTruthy(env.CONTRACTS_V2_LOCAL_DATABASE)) {
    return {
      status: 'CONTRACTS_V2_LOCAL_DATABASE_REQUIRED',
      startedAt,
      finishedAt: new Date().toISOString(),
      migrationsAppliedLocally: false,
      migrationsAppliedRemotely: false,
      reason: 'CONTRACTS_V2_LOCAL_DATABASE must be true',
    };
  }
  ...
  if (!isTruthy(env.RUN_SUPABASE_LOCAL_INTEGRATION)
    ...
    return {
      ...
      migrationsAppliedLocally: false,
      migrationsAppliedRemotely: false,
      note: 'Opt-in incompleto (integration + confirmation + APPLY_LOCAL_DB_RESET + CONTRACTS_V2_LOCAL_DATABASE)',
    };
  }
  ...
  const missing = REQUIRED_MIGS.filter((m) => !fs.existsSync(path.join(ISOLATED_DIR, 'supabase/migrations', m)));
  const checksumOk = checksums.status === 'ISOLATED_MIGRATION_CHECKSUM_OK';
  if (missing.length || !checksumOk) {
    return {
  ...
  // reset #1
  const reset1 = await runGuarded(binary, ['db', 'reset', '--yes'], env, commandsExecuted, 600_000);
  if (reset1.blocked || reset1.result?.exitCode !== 0) {
    return {
      status: 'RESET_FAILED',
      commandsExecuted,
      startedAt,
  ...
  if (!resolved.container) {
    return {
      status: 'LOCAL_DB_CONTAINER_NOT_FOUND',
      commandsExecuted,
      startedAt,
      finishedAt: new Date().toISOString(),
      migrationsAppliedRemotely: false,
  ...
  const containerPath = '/tmp/contracts_v2_phase109_validation.sql';
  await runGuarded(
    'docker',
  ...
  const out1 = String(run1.result?.stdoutSanitized || '') + String(run1.result?.stderrSanitized || '');
  const pass1 = out1.includes('CONTRACTS_V2_LOCAL_PASS') && run1.result?.exitCode === 0;

  // reset #2 + re-run (reprodutibilidade)
  const reset2 = await runGuarded(binary, ['db', 'reset', '--yes'], env, commandsExecuted, 600_000);
  await runGuarded(
    'docker',
    ['cp', FIXTURE_ABS, `${resolved.container}:${containerPath}`],
    env,
  ...
  const out2 = String(run2.result?.stdoutSanitized || '') + String(run2.result?.stderrSanitized || '');
  const pass2 = out2.includes('CONTRACTS_V2_LOCAL_PASS') && run2.result?.exitCode === 0;

  // schema evidence: migrations applied
  const migProbe = await runGuarded(
    'docker',
    [
      'exec', resolved.container, 'psql', '-U', 'postgres', '-d', 'postgres', '-At', '-c',
      "select version from supabase_migrations.schema_migrations where version like '028%' or version like '029%' or version like '030%' or version like '031%' order by version;",
    ],
    env,
    commandsExecuted,
    20_000,
  ...
  const status = pass1 && pass2 && reset2.result?.exitCode === 0
    ? 'CONTRACTS_V2_LOCAL_PASS'
    : 'CONTRACTS_V2_LOCAL_FAILED';
  ...
  const report = {
    ...
    migrationsAppliedLocally: true,
    migrationsAppliedRemotely: false,
    migrations: REQUIRED_MIGS,
    firstPass: pass1,
    secondPass: pass2,
    reproducibility: pass1 && pass2,
    ...
    note: 'Stack local permanece up para auditoria; use supabase stop no workdir supabase-local para teardown.',
  };

  if (asJson) {
    process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
  } else {
    console.log(`[phase109] status=${report.status} pass1=${pass1} pass2=${pass2}`);
    console.log(`[phase109] migrations local=true remote=false`);
    console.log(`[phase109] versions:\n${report.migrationVersionsProbe}`);
  }
  return report;
}
...
if (isMain) {
  runLocalContractsV2Validation({ json: process.argv.includes('--json') })
    .then((r) => {
      process.exit(r.status === 'CONTRACTS_V2_LOCAL_PASS' ? 0 : 1);
    })
    .catch((err) => {
      console.error(err);
      process.exit(1);
