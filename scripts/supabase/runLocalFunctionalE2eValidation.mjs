 ...
 *
 *   node scripts/supabase/runLocalFunctionalE2eValidation.mjs
 *   node scripts/supabase/runLocalFunctionalE2eValidation.mjs --json
 */
...
import { PHASE_9_3A_DOMAIN_MAP, PHASE_9_3A_TEST_LAYER } from './phase93aDomainMap.mjs';
import { guardCommand } from './remoteGuard.mjs';
import { checkCli, checkDocker, resolveSupabaseBinary } from './toolchainPreflight.mjs';

const FIXTURE_REL = path.join('fixtures', 'functional_e2e_validation.sql');
...
export async function runLocalFunctionalE2eValidation(options = {}) {
  ...
  const base = {
    ...
    domainMap: PHASE_9_3A_DOMAIN_MAP,
    testLayer: PHASE_9_3A_TEST_LAYER,
    commandsExecuted,
    blockers: [],
    warnings: [],
    scenarios: [],
    outOfScope: PHASE_9_3A_DOMAIN_MAP.filter((d) => !d.inScopeSqlE2e).map((d) => d.domain),
    durationMs: 0,
  };

  if (optIn.status !== 'OPT_IN_OK') {
    base.blockers.push(...optIn.blockers);
    base.status = 'FUNCTIONAL_E2E_SKIPPED_OPT_IN';
  ...
  if (!fs.existsSync(FIXTURE_ABS)) {
    base.blockers.push('FUNCTIONAL_E2E_FIXTURE_MISSING');
  }
  if (isTruthy(env.APPLY_LOCAL_DB_RESET)) {
    base.warnings.push('APPLY_LOCAL_DB_RESET is set but functional-e2e never resets the DB');
  }
  ...
  if (base.blockers.length) {
    base.status = 'FUNCTIONAL_E2E_BLOCKED';
    base.durationMs = Date.now() - started;
    return base;
  }
  ...
  const queryResult = await runProcess('docker', psqlArgs, {
    cwd: ISOLATED_DIR,
    env,
    timeoutMs: Number(env.SUPABASE_FUNCTIONAL_E2E_TIMEOUT_MS) || 180_000,
    stdin: sql,
  });
  commandsExecuted.push({
    blocked: false,
  ...
  const hasPass = /FUNCTIONAL_E2E_PASS/.test(out);
  const hasFail = /FUNCTIONAL_E2E_FAILED/.test(out);

  if (timedOut) {
    base.blockers.push('FUNCTIONAL_E2E_TIMEOUT');
    base.status = 'FUNCTIONAL_E2E_FAILED';
  } else if (hasPass && exitOk && !hasFail) {
    base.status = 'FUNCTIONAL_E2E_PASS';
  } else if (/schema_missing_run_dry_run_first/i.test(out)) {
    base.blockers.push('SCHEMA_NOT_APPLIED_RUN_LOCAL_DRY_RUN_FIRST');
    base.status = 'FUNCTIONAL_E2E_BLOCKED';
  } else if (/connection refused|No such container/i.test(out)) {
    base.blockers.push('LOCAL_DB_QUERY_FAILED');
    base.status = 'FUNCTIONAL_E2E_BLOCKED';
  } else {
    base.blockers.push('FUNCTIONAL_E2E_ASSERTIONS_FAILED');
    base.status = 'FUNCTIONAL_E2E_FAILED';
  }
  ...
  base.scenarios = out
    .split(/\r?\n/)
    .filter((l) =>
      /precondition_|out_of_scope_|tenant_|iso_|storage_|budget_|journey_|schema_/.test(l))
    .slice(0, 120);

  base.warnings.push(
    'Patients/budgets/journey/payments/odontogram/operational-audit: OUT_OF_SCOPE_IDB (sem tabela SQL local)',
  );
  base.warnings.push(
    'Fluxo SSOT executado via SQL fixture + JWT/RLS; Admin API flags default-off não foram reimplementadas',
  );
...
if (isMain) {
  const report = await runLocalFunctionalE2eValidation();
  if (process.argv.includes('--json')) {
    process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
  } else {
    console.log(`Phase 9.3A functional E2E: ${report.status}`);
    ...
    console.log(`outOfScope=${(report.outOfScope || []).join(',')}`);
    ...
    if (report.scenarios?.length) {
      console.log('--- scenarios (sample) ---');
      for (const line of report.scenarios.slice(0, 40)) console.log(line);
    }
  }
  if (report.status === 'FUNCTIONAL_E2E_PASS') process.exit(0);
  if (
    report.status === 'FUNCTIONAL_E2E_SKIPPED_OPT_IN'
    || report.status === 'FUNCTIONAL_E2E_BLOCKED'
  ) {
    process.exit(2);
  }
  process.exit(1);
