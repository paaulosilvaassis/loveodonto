 ...
 * Pré-requisito: migrations 025–027 aplicadas (dry-run local).
 *
 *   node scripts/supabase/runLocalPatientsWave2RlsValidation.mjs
 *   node scripts/supabase/runLocalPatientsWave2RlsValidation.mjs --json
 */
...
const FIXTURE_REL = path.join('fixtures', 'patients_wave2_rls_validation.sql');
...
export async function runLocalPatientsWave2RlsValidation(options = {}) {
  ...
  const base = {
    phase: '9.4A-Wave2',
    status: 'PATIENTS_WAVE2_RLS_BLOCKED',
  ...
  if (optIn.status !== 'OPT_IN_OK') {
    ...
    base.status = 'PATIENTS_WAVE2_RLS_SKIPPED_OPT_IN';
  ...
  if (!fs.existsSync(FIXTURE_ABS)) {
    base.blockers.push('PATIENTS_WAVE2_RLS_FIXTURE_MISSING');
  }
  ...
  if (base.blockers.length) {
    base.status = 'PATIENTS_WAVE2_RLS_BLOCKED';
    base.durationMs = Date.now() - started;
    return base;
  }
  ...
  const queryResult = await runProcess('docker', psqlArgs, {
    cwd: ISOLATED_DIR,
    env,
    timeoutMs: Number(env.SUPABASE_PATIENTS_WAVE2_RLS_TIMEOUT_MS) || 120_000,
    stdin: sql,
  });
  commandsExecuted.push({
    blocked: false,
  ...
  const hasPass = /PATIENTS_WAVE2_RLS_PASS/.test(out);
  const hasFail = /PATIENTS_WAVE2_RLS_FAILED/.test(out);

  if (timedOut) {
    base.blockers.push('PATIENTS_WAVE2_RLS_QUERY_TIMEOUT');
    base.status = 'PATIENTS_WAVE2_RLS_FAILED';
  } else if (hasPass && exitOk && !hasFail) {
    base.status = 'PATIENTS_WAVE2_RLS_PASS';
  } else if (
    /schema_gap_patients_wave1_missing|rode.*dry-run/i.test(out)
    || /precondition_patients_exists\s*\|\s*f\b/i.test(out)
  ) {
    base.blockers.push('SCHEMA_NOT_APPLIED_RUN_LOCAL_DRY_RUN_WITH_025');
    base.status = 'PATIENTS_WAVE2_RLS_BLOCKED';
  } else if (/connection refused|No such container|does not exist/i.test(out)) {
    base.blockers.push('LOCAL_DB_QUERY_FAILED');
    base.status = 'PATIENTS_WAVE2_RLS_BLOCKED';
  } else {
    base.blockers.push('PATIENTS_WAVE2_RLS_ASSERTIONS_FAILED');
    base.status = 'PATIENTS_WAVE2_RLS_FAILED';
  }
...
if (isMain) {
  const report = await runLocalPatientsWave2RlsValidation();
  if (process.argv.includes('--json')) {
    process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
  } else {
    console.log(`Phase 9.4A Wave1 Patients RLS: ${report.status}`);
  ...
  if (report.status === 'PATIENTS_WAVE2_RLS_PASS') process.exit(0);
  if (
    report.status === 'PATIENTS_WAVE2_RLS_SKIPPED_OPT_IN'
    || report.status === 'PATIENTS_WAVE2_RLS_BLOCKED'
  ) {
    process.exit(2);
  }
  process.exit(1);
