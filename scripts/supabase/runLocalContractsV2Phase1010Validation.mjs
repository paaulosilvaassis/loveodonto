/**
 * Phase 10.10 — apply local 028–033 + validação sessions/challenges/bucket.
 *
 * Requer:
 ...
 *
 *   node scripts/supabase/runLocalContractsV2Phase1010Validation.mjs
 */
...
const FIXTURE_REL = path.join('fixtures', 'contracts_v2_phase1010_validation.sql');
...
export async function runLocalContractsV2Phase1010Validation(options = {}) {
  ...
  if (!isTruthy(env.CONTRACTS_V2_LOCAL_DATABASE) || !isTruthy(env.CONTRACTS_V2_LOCAL_STORAGE)) {
    return {
      ...
      reason: 'CONTRACTS_V2_LOCAL_DATABASE + CONTRACTS_V2_LOCAL_STORAGE must be true',
    };
  }
  ...
  async function resetAndValidate() {
    ...
    const containerPath = '/tmp/contracts_v2_phase1010_validation.sql';
    await runGuarded(
      'docker',
    ...
    const pass = out.includes('CONTRACTS_V2_PHASE1010_PASS') && run.result?.exitCode === 0;
  ...
  const migProbe = first.resolved?.container
    ? await runGuarded(
      ...
      [
        ...
        "select version from supabase_migrations.schema_migrations where version ~ '^(028|029|030|031|032|033)' order by version;",
      ],
      env,
      commandsExecuted,
      20_000,
  ...
  const bucketProbe = first.resolved?.container
    ? await runGuarded(
      'docker',
      [
        'exec', first.resolved.container, 'psql', '-U', 'postgres', '-d', 'postgres', '-At', '-c',
        "select id || ':' || public::text from storage.buckets where id = 'contracts-v2-private-local';",
      ],
      env,
      commandsExecuted,
      20_000,
  ...
  const status = first.ok && second.ok ? 'CONTRACTS_V2_PHASE1010_PASS' : 'CONTRACTS_V2_PHASE1010_FAILED';
  const report = {
    status,
    startedAt,
    finishedAt: new Date().toISOString(),
    ...
    bucketProbe: String(bucketProbe?.result?.stdoutSanitized || '').trim(),
    ...
    note: 'Stack local permanece up; supabase stop no workdir supabase-local para teardown.',
  };

  if (asJson) {
    process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
  } else {
    console.log(`[phase1010] status=${report.status} pass1=${first.ok} pass2=${second.ok}`);
    console.log(`[phase1010] migrations local=true remote=false bucket=${report.bucketProbe}`);
    console.log(`[phase1010] versions:\n${report.migrationVersionsProbe}`);
  }
  return report;
}
...
if (isMain) {
  runLocalContractsV2Phase1010Validation({ json: process.argv.includes('--json') })
    .then((r) => {
      process.exit(r.status === 'CONTRACTS_V2_PHASE1010_PASS' ? 0 : 1);
    })
    .catch((err) => {
      console.error(err);
      process.exit(1);
