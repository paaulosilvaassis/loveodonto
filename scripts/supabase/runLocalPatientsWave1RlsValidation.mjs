 ...
 *
 *   node scripts/supabase/runLocalPatientsWave1RlsValidation.mjs
 *   node scripts/supabase/runLocalPatientsWave1RlsValidation.mjs --json
 */
...
const FIXTURE_REL = path.join('fixtures', 'patients_wave1_rls_validation.sql');
...
export async function runLocalPatientsWave1RlsValidation(options = {}) {
  ...
  const base = {
    phase: '9.4A-Wave1',
    status: 'PATIENTS_WAVE1_RLS_BLOCKED',
  ...
  if (optIn.status !== 'OPT_IN_OK') {
    ...
    base.status = 'PATIENTS_WAVE1_RLS_SKIPPED_OPT_IN';
  ...
  if (!fs.existsSync(FIXTURE_ABS)) {
    base.blockers.push('PATIENTS_WAVE1_RLS_FIXTURE_MISSING');
  }
  ...
  if (base.blockers.length) {
    base.status = 'PATIENTS_WAVE1_RLS_BLOCKED';
    base.durationMs = Date.now() - started;
    return base;
  }
  ...
  const queryResult = await runProcess('docker', psqlArgs, {
    cwd: ISOLATED_DIR,
    env,
    timeoutMs: Number(env.SUPABASE_PATIENTS_WAVE1_RLS_TIMEOUT_MS) || 120_000,
    stdin: sql,
  });
  commandsExecuted.push({
    blocked: false,
  ...
  const hasPass = /PATIENTS_WAVE1_RLS_PASS/.test(out);
  const hasFail = /PATIENTS_WAVE1_RLS_FAILED/.test(out);

  if (timedOut) {
    base.blockers.push('PATIENTS_WAVE1_RLS_QUERY_TIMEOUT');
    base.status = 'PATIENTS_WAVE1_RLS_FAILED';
  } else if (hasPass && exitOk && !hasFail) {
    base.status = 'PATIENTS_WAVE1_RLS_PASS';
  } else if (
    ...
    base.blockers.push('PATIENTS_WAVE1_RLS_ASSERTIONS_FAILED');
    base.status = 'PATIENTS_WAVE1_RLS_FAILED';
  }
...
if (isMain) {
  const report = await runLocalPatientsWave1RlsValidation();
  ...
  if (report.status === 'PATIENTS_WAVE1_RLS_PASS') process.exit(0);
  if (
    report.status === 'PATIENTS_WAVE1_RLS_SKIPPED_OPT_IN'
    || report.status === 'PATIENTS_WAVE1_RLS_BLOCKED'
  ) {
    process.exit(2);
  }
  process.exit(1);
