#!/usr/bin/env node
/**
 * PHASE_10.21AB — Full draft→signed package E2E (staging browser).
...
const OUT = path.join(ROOT, 'docs/reports/_phase1021ab_full_e2e.json');
...
async function main() {
  ...
  try {
    ...
    // Send for signature (UI)
    await clickFirst(page, /Enviar para assinatura/i);
    await page.waitForTimeout(800);
    ...
    if (!sendInfo.hasLink) {
      const svcSend = await page.evaluate(async ({ contractId }) => {
        ...
        try {
          ...
          const budget = getBudget(c.quoteId) || {
            id: c.budgetId,
    ...
    // View each document — discover keys from DOM
    const docKeys = await page.locator('[data-testid^="pkg-doc-"]').evaluateAll((nodes) => (
      nodes.map((n) => (n.getAttribute('data-testid') || '').replace(/^pkg-doc-/, '')).filter(Boolean)
    ));
    ...
    for (const key of docKeys) {
      ...
      if (/contract/i.test(key)) report.CONTRACT_VIEW = visible && text.length > 10 ? 'PASS' : 'FAIL';
      if (/tcle/i.test(key)) report.TCLE_VIEW = visible && text.length > 10 ? 'PASS' : 'FAIL';
      if (/lgpd/i.test(key)) report.LGPD_VIEW = visible && /LGPD|privacidade|dados/i.test(text) ? 'PASS' : 'FAIL';
      await btn.click().catch(() => {});
    }

    // Sign gate A/B/C using discovered keys
    const contractKey = docKeys.find((k) => /contract/i.test(k));
    const tcleKey = docKeys.find((k) => /tcle/i.test(k));
    const lgpdKey = docKeys.find((k) => /lgpd/i.test(k));

    async function acceptDoc(key) {
      const row = page.locator(`[data-testid="pkg-doc-${key}"]`);
      await row.locator('button', { hasText: /Visualizar/i }).click();
      await page.waitForTimeout(200);
      await row.locator(`input[data-testid="pkg-doc-accept-${key}"]`).check({ force: true });
      await page.waitForTimeout(500);
    }
    ...
    const signed = await page.evaluate(async ({ contractId, patientId }) => {
      ...
      const files = (db.patientFiles || db.files || []).filter((f) => (
        (f.patientId === patientId || f.patientId === c?.patientId) && f.metadata?.signedPackage
      ));
    ...
    await page.screenshot({ path: path.join(ROOT, 'docs/reports/_phase1021ab_full_e2e.png'), fullPage: false });
  } catch (e) {
    report.error = String(e.message || e).slice(0, 600);
    note('fatal', false, report.error);
    await page.screenshot({ path: path.join(ROOT, 'docs/reports/_phase1021ab_full_e2e.png'), fullPage: false }).catch(() => {});
  }

  const checks = [
    'DRAFT_GENERATION', 'CONTRACT_FINALIZE', 'LGPD_PACKAGE_DOCUMENT', 'PACKAGE_3_DOCUMENTS',
  ...
  report.allPass = checks.every((k) => report[k] === 'PASS');
