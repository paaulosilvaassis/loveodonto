...
/**
 * @returns {{ ip: string, source: string, environment: string, forwardedIgnored: boolean }}
 */
...
const TRUSTED_SOURCES = new Set(['trusted-proxy', 'socket', 'server']);

/**
 * IP da evidência: só aceita contexto observado no servidor.
 * String solta do frontend é ignorada em produção.
 */
export function resolveEvidenceClientIp({
  observedClientContext = null,
  fallbackIp = '',
  runtime = typeof process !== 'undefined' ? process.env.NODE_ENV : '',
} = {}) {
  const source = observedClientContext?.source;
  const observed = normalizeIp(observedClientContext?.ip);
  if (TRUSTED_SOURCES.has(source) && observed && observed !== 'local' && isValidIp(observed)) {
    return { ip: observed, source };
  }
  if (source === 'local-dev' || observed === 'local') {
    return { ip: 'local', source: 'local-dev' };
  }
  const isDev = runtime === 'test' || runtime === 'development' || runtime === undefined;
  if (isDev && fallbackIp) {
    return { ip: normalizeIp(fallbackIp) || 'local', source: 'local-dev' };
  }
  if (isDev) return { ip: 'local', source: 'local-dev' };
  return { ip: 'unavailable', source: 'unavailable' };
}
