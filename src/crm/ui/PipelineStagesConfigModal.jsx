...
const newDraftStage = () => ({
  ...
  id: null,
  label: '',
  color: STAGE_COLOR_PRESETS[(draftSeq + 3) % STAGE_COLOR_PRESETS.length],
  isActive: true,
  stageType: STAGE_TYPE.NORMAL,
});

/**
 * Configuração das fases do pipeline: criar, renomear, recolorir, reordenar,
 * ativar/desativar, definir tipo (normal/conversão/perda) e excluir fases vazias.
 * Salva tudo de uma vez por tenant.
 */
export function PipelineStagesConfigModal({ open, onClose, user, stages, leadCounts = {}, onSaved }) {
  ...
  const handleAdd = () => {
    setError('');
    setDrafts((prev) => [...prev, newDraftStage()]);
  };
  ...
  return (
    <ModalRoot open={open} onOpenChange={(next) => { if (!next) onClose(); }}>
      <ModalContent size="lg" onInteractOutside={(e) => e.preventDefault()}>
        <ModalHeader>
          <ModalTitle>Configurar fases do pipeline</ModalTitle>
          <ModalDescription>
            Personalize o funil da sua clínica: nome, cor, ordem, status e tipo de cada fase.
            É obrigatório manter ao menos uma fase ativa, uma de conversão e uma de perda.
          </ModalDescription>
        </ModalHeader>
        <ModalBody>
          {error && (
            <div className="crm-stages-config-error" role="alert">{error}</div>
          )}
          <div className="crm-stages-config-list">
            {drafts.map((draft, index) => (
              <div key={draft.draftId} className={`crm-stages-config-row ${draft.isActive ? '' : 'is-inactive'}`.trim()}>
                ...
                <span className="crm-stages-config-count" title="Leads nesta fase">
                  {countFor(draft)} lead(s)
                </span>
                <label className="crm-stages-config-active">
                  <input
                    type="checkbox"
                    checked={draft.isActive}
                    onChange={() => handleToggleActive(draft)}
                  />
                  Ativa
                </label>
                <button
                  ...
                  aria-label={`Excluir fase ${draft.label || index + 1}`}
                  disabled={countFor(draft) > 0}
                  title={countFor(draft) > 0 ? 'Apenas fases vazias podem ser excluídas' : 'Excluir fase'}
          ...
          <Button type="button" variant="ghost" size="sm" icon={Plus} onClick={handleAdd}>
            Adicionar fase
          </Button>
        </ModalBody>
        <ModalFooter>
