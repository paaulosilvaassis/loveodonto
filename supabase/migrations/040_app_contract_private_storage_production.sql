-- =============================================================================
-- 040: Contracts V2 private storage — PRODUCTION
-- =============================================================================
-- STATUS: PREPARED in PHASE_10.21AG — DO NOT APPLY until explicit human
-- authorization in a later phase.
--
-- Bucket: contracts-v2-private-production
-- NÃO criar bucket local (contracts-v2-private-local).
-- NÃO criar bucket staging (contracts-v2-private-staging).
-- NÃO aplicar 033 nem 035.
--
-- Inclui schema de lifecycle de arquivos (033/035 SKIPPED em production)
-- + bucket privado production + policies deny-by-default para writes.
-- =============================================================================
...
comment on table public.app_contract_storage_ops is
  'PHASE_10.21AG/040 — eventos operacionais de storage production; sem bytes/token/signed URL/PII.';
...
-- ---------------------------------------------------------------------------
-- Path helpers — tenants/{tenant}/contracts/{contract}/versions/{version}/...
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Bucket privado PRODUCTION
-- ---------------------------------------------------------------------------

insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values (
  'contracts-v2-private-production',
  'contracts-v2-private-production',
  false,
  20971520,
  array[
...
drop policy if exists contracts_v2_private_production_select on storage.objects;
drop policy if exists contracts_v2_private_production_insert on storage.objects;
drop policy if exists contracts_v2_private_production_update on storage.objects;
drop policy if exists contracts_v2_private_production_delete on storage.objects;

-- SELECT tenant-scoped + path canônico. INSERT/UPDATE/DELETE authenticated: ausentes.
create policy contracts_v2_private_production_select on storage.objects
  for select
  to authenticated
  using (
    bucket_id = 'contracts-v2-private-production'
